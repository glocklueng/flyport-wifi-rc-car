 ...
 * \paragraph usage Usage
 ...
 <UL>
	...
	<LI><B>Network type (adhoc or infrastructure):</B> WFSetParam(NETWORK_TYPE , ADHOC or INFRASTRUCTURE)</LI> <BR>
 </ul>
 */
		...
		for (ind1 = 0; ind1 < 15; ind1++)
		{
			if ( (paramstring[ind1] != '\0') && (!nbns_flag) )
			{
				...
				if ( (paramstring[ind1] > 96) && (paramstring[ind1] < 123) )
					WFConf[0].NetBIOSName[ind1] = paramstring[ind1]-32;
				else
					WFConf[0].NetBIOSName[ind1] = paramstring[ind1];
			}
			else if (paramstring[ind1] == '\0')
				nbns_flag = TRUE;
	
			if (nbns_flag)
				WFConf[0].NetBIOSName[ind1] = 0x0020;
		}
		WFConf[0].NetBIOSName[15] = '\0';
		break;
	//	SSID name
	case SSID_NAME:
