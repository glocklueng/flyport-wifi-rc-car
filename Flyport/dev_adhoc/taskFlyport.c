...
/* The servo is controled by pulse lenght, not duty cycle. These are maximum values for our HiTec servomotor. */
...
void FlyportTask()
{
	/* Variables déclaration */
		...
		DWORD CurrentTick; // timer related variables
	...
	/* Wi-Fi connection */
		...
		while (WFStatus == CONNECTING);
		
		if (WFStatus == CONNECTED)
		{
			UARTWrite(1,"Flyport is now connected with infrastructure settings.\r\n");
		}
		else
		{
			...
			NETSetParam(DHCP_ENABLE, "ENABLED");		// ENABLED ou DISABLED
			NETSetParam(SSID_NAME, "Flyport");
			NETSetParam(NETWORK_TYPE, "ADHOC");// ADHOC ou INFRASTRUCTURE
			WFConnect(WF_CUSTOM);
			while (WFStatus != CONNECTED);
			UARTWrite(1,"Flyport is now connected with adhoc settings.\r\n");
		}
		vTaskDelay(400);
    ...
    while(1)
    {
		...
		CurrentTick = TickGet();
		ElapsedTimeMs = TickConvertToMilliseconds(CurrentTick - StartTime);
		if (ElapsedTimeMs < 20) {
